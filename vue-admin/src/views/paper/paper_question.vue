    ...
    <el-form label-width="120px">
      ...
      <el-main class="main" >
        <p class="zujuan">组卷方式</p>
          <div  class="div1">
              <!-- <router-link to="/paper/paper_question/'">  -->
          
        <el-radio v-model="radio1" label="1" border @change="zidingyibtn" class="el-radio">自定义组卷</el-radio> </router-link> 
         <el-radio v-model="radio1" label="2" border @change="suijibtn">随机组卷</el-radio> 
          </div>
           <div  v-show="zidingyi">
               <div class="div2">
                 <p>
                 ...
                 <el-button class="el-button3" round type="primary" size="small " @click="choice_question"><span class="icon iconfont icon-jiahao"></span> 添加试题</el-button>
                 ...
                 <div class="number">
                        单选题:<span class="gongji">{{Choicetotal}}</span>&#8197;题   &#8197;
                        判断题:<span class="gongji">{{Judgetotal}}</span>&#8197;题   &#8197;
                   现在共计<span class="gongji">{{total}}</span>&#8197;题
                 </div>
                 <!-- 单选 -->
                  <div v-show="Choicetotal>0">
                 <el-collapse v-for="(choiceQuestion,index) in choiceQuestions"  :key="choiceQuestion.id" v-show="danxuan">
                   <input type="checkbox" class="checkbox" :value="choiceQuestion.id" v-model="checkedChoiceQuestions" >
              ...
              <div  v-if="choiceQuestions==0&&Judgetotal==0">
               <img src="/static/kong.png">
               <div class="tips">点击<span class="tip">“添加试题“</span>,可题库随机选题、手工选题、直接编辑试题、Excel文档一键导入，每个人考试的试题一样！</div>
              </div>
                 <!-- 判断 -->
                 <el-collapse v-for="(choiceQuestion,index) in judgeQuestion"  :key="choiceQuestion.id" v-show="panduan">
               ...
               <img src="/static/kong.png" >
               <div class="tips">点击<span class="tip">“添加试题“</span>,可题库随机选题、手工选题、直接编辑试题、Excel文档一键导入，每个人考试的试题一样！</div>
               <el-button class="el-button2" type="primary" ><span class="icon iconfont icon-jiahao"></span> 添加试题</el-button>
               </div> -->
          </div>
          <!-- 随机组卷 -->
          <div  v-show="suiji">
          ...
          <div class="time2">{{paper.paperName}}</div>
            <div class="time">
               ...
               <span class="time5">类型名称</span>
                 ...
                 <div class="time8">{{paper.bankName}}</div>
      ...
      panduanti(){
       ...
       deletePaper(){
          ...
          //  console.log(this.checkedJudgeQuestions)
           if(this.checkedJudgeQuestions.length!=0){
             ...
             // 单选
          if(this.checkedChoiceQuestions.length!=0){
           this.$confirm('确定删除选中的这些试题吗？, 是否继续?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
            }).then(() => {
              paperQuestion.removeBath(this.checkedChoiceQuestions).then(res=>{
            // console.log(res);
               this.listQuestion()
               this.listJudgeQuestion()
                this.$message({
       ...
       suijibtn(){
          this.$confirm('确定跳转到随机组卷页面吗？跳转到随机组卷页面将删除您刚才自定义的所有试卷题目, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    showClose:false,
                    closeOnClickModal:false
                  }).then(() => {
                    paperQuestion.deleteByPaperId(this.paperId,1).then(res=>{
