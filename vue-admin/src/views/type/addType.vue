<template>
 ...
 <el-col :span="8" style="margin-top:70px">
     <p><b style="font-size:20px;margin-left:170px;">添加信息类型</b></p>
 <el-form :model="notice" :rules="rules" ref="notice" label-width="100px" class="demo-ruleForm">
    <el-form-item label="信息类型" prop="noticeTitle"  class="aaa">
      <el-input v-model="notice.type" class="option-input" placeholder="请输入信息类型"></el-input>
    </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      updateNotice(){
        ...
        type.updateNotice(this.notice).then(res=>{
          console.log(res);
              if (res.data.repeat==='repeat'){
                ...
                this.$alert('修改成功,点击跳转到信息类型界面', '消息', {
                  confirmButtonText: '确定',
                  callback: action => {
      ...
      addNotice(){
          console.log(this.notice);
        type.addNotice(this.notice).then(res=>{
          // console.log(res);
              if (res.data.repeat==='repeat'){
                ...
                this.$alert('添加成功,点击跳转到信息类型页面', '消息', {
                  confirmButtonText: '确定',
                  callback: action => {
                    this.$router.push("/type/index");
                  }
      ...
      getOneById(id){
          type.listById(id).then(res=>{
            // console.log(res);
            this.notice=res.data.type
          })
      },
      init(){
