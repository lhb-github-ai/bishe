<template>
 <div style="background-color: #eee;">
    <div style="font-size:23px;">
      ...
      <p v-show="flag2">分数:<span style="color: red;font-size: 23px">{{exams.score}}</span></p>
    </div>
    <br>
    <br>
    <br>
    <ul style="margin-left:27%;width:45%;">
      ...
      <li id="qu_0_0" highlight-current-row v-loading="loading" v-for="(wor1, index1) in topicDX" :key="wor1.id"
        ...
        <div  class="test_content_nr_tt"><i >{{index1+1}}</i> <span >{{wor1.questionName}}</span> </div>
              ...
              <div v-show="falg">
                ...
                <div style="font-size: 17px;border: solid 1px red;">
                  ...
                  <div v-if="(wor1.answer)!=(wor1.options)" style="color: red;font-weight:bold;font-size: 20px"
                      >错误</div>
                  正确答案:<span >{{wor1.answer}}</span> 你的答案:<span >{{wor1.options}}</span></div>
              </div>
            </li>
          </ul>
        </div>
      ...
      <li id="qu_0_0" highlight-current-row v-loading="loading" v-for="(wor2, index) in topicPD" :key="wor2.id"
        ...
        <div  class="test_content_nr_tt"> <i >{{topicPD.length+index+2}}</i> <span >{{wor2.questionName}}</span></div>
        <div class="test_content_nr_main">
          <ul>
            <li class="option">
            ...
            <li class="option">
              ...
              <div v-show="falg">
                <br>
                <div style="font-size: 17px;border: solid 1px red;">
                  <div v-if="(wor2.answer)==(wor2.options)" style="color: green;font-weight:bold;font-size: 20px "
                       >正确</div>
                  <div v-if="(wor2.answer)!=(wor2.options)" style="color: red;font-weight:bold;font-size: 20px"
                      >错误</div>
                  正确答案:<span >{{wor2.answer}}</span> 你的答案:<span >{{wor2.options}}</span></div>
              </div>
            </li>
          </ul>
        </div>
    ...
    <br>
    <br>
    <el-link href="/testPaper" type="primary" icon="el-icon-back" style="float: left;font-size: 17px">返回测试中心
    </el-link>
      <el-tooltip placement="bottom" effect="light">
        <div slot="content" >{{app}}</div>
        <el-button type="primary" style="margin-left:40%" @click="commit()" :disabled="flag">提交考试</el-button>
      </el-tooltip>
    <br>
    <br>
  </div>
 ...
 export default {
    ...
    methods:{
      ...
      commit(){
         ...
         this.flag = true;
       let finalScore= 0;
        for (let i = 0; i < this.topicDX.length; i++) {
          if (this.topicDX[i].options == this.topicDX[i].answer) {
           finalScore += (this.examData.totalScore/2)/this.topicDX.length
            
          } else {
            // console.log("错误")
          }
        }
     console.log("单选题分"+finalScore);
   for (let i = 0; i < this.topicPD.length; i++) {
          if (this.topicPD[i].options == this.topicPD[i].answer) {
           finalScore += (this.examData.totalScore/2)/this.topicPD.length
            
          } else {
            // console.log("错误")
          }
        }
        console.log("总分"+finalScore);
